...
<div class="dpimages-icons-box">
  {foreach from=$images item=i name=images}
    ...
    <a href="{$i.image_url|amp}" class="cloud-zoom-gallery" id="dpicon_{$smarty.foreach.images.index}" rel="useZoom: 'cloud_zoom_image', smallImage: '{$i.thbn_url|escape:"javascript"}'" title="{$i.alt|escape}" onclick='javascript:detailed_images("{$i.alt|escape}"); cloudzoom_change_image_size({$i.thbn_image_x}, {$i.thbn_image_y}); cloudzoom_change_popup_params("{$cloudzoom_selected_image_popup_params|escape}");{$smarty.capture.switch_image_box}'><img src="{$i.icon_url|amp}" alt="{$i.alt|escape}" title="{$i.alt|escape}" width="{$i.icon_image_x}" height="{$i.icon_image_y}" /></a>
    {/if}
    {/foreach}
